...
lint/isort: ## check style with flake8
	isort --check psy_maps tests
lint/flake8: ## check style with flake8
	flake8 psy_maps tests
lint/black: ## check style with black
	black --check psy_maps tests
	blackdoc --check psy_maps tests
...
formatting:
	isort psy_maps tests
	black psy_maps tests
	blackdoc psy_maps tests

quick-test: ## run tests quickly with the default Python
	python -m pytest

pipenv-test: ## run tox
	pipenv run isort --check psy_maps
	pipenv run black --line-length 79 --check psy_maps
	pipenv run flake8 psy_maps
	pipenv run pytest -v --cov=psy_maps -x --ref
	pipenv run pytest -v --cov=psy_maps -x --cov-append -n $(NPROCS)
...
coverage: ## check code coverage quickly with the default Python
	python -m pytest --cov psy_maps --cov-report=html
