...
from psy_maps.plotters import FieldPlotter, InteractiveList, rcParams
...
class FieldPlotterTest(tb.BasePlotterTest, MapReferences):
    ...
    def test_lonlatbox(self, *args):
        ...
        for data in all_data:
            ...
            self.assertLessEqual(
                data.lon.values.max(), 97.0, msg=msg % ("longitude", "maximum")
            )
            self.assertGreaterEqual(
                data.lat.values.min(), -8.0, msg=msg % ("latitude", "minimum")
            )
            self.assertLessEqual(
                data.lat.values.max(), 81.0, msg=msg % ("latitude", "maximum")
            )
    ...
    def test_projection(self, *args):
        ...
        self.compare_figures(
            next(iter(args), self.get_ref_file("projection1"))
        )
        self.update(projection="northpole")
        self.compare_figures(
            next(iter(args), self.get_ref_file("projection2"))
        )
        self.update(projection=ccrs.LambertConformal())
        self.compare_figures(
            next(iter(args), self.get_ref_file("projection3"))
        )
        self.update(
            projection="rotated", lonlatbox="Europe", map_extent="data"
        )
        self.compare_figures(
            next(iter(args), self.get_ref_file("projection4"))
        )
    ...
    def test_grid(self, *args):
        ...
        self.compare_figures(
            next(iter(args), self.get_ref_file("grid1")), tol=10
        )
        self.update(xgrid="rounded", ygrid=["data", 20])
        self.compare_figures(
            next(iter(args), self.get_ref_file("grid2")), tol=10
        )
        self.update(xgrid=True, ygrid=True, grid_color="w")
        self.compare_figures(
            next(iter(args), self.get_ref_file("grid3")), tol=10
        )
        self.update(
            xgrid=True,
            ygrid=True,
        ...
        self.compare_figures(
            next(iter(args), self.get_ref_file("grid4")), tol=10
        )

    def test_clon(self):
        """Test clon formatoption"""
        self.update(clon=180.0)
        self.assertEqual(
            self.plotter.ax.projection.proj4_params["lon_0"], 180.0
        )
        self.update(clon="India")
        self.assertEqual(
            self.plotter.ax.projection.proj4_params["lon_0"], 82.5
        )

    def test_clat(self):
        """Test clat formatoption"""
        self.update(projection="ortho", clat=60.0, grid_labels=False)
        self.assertEqual(
            self.plotter.ax.projection.proj4_params["lat_0"], 60.0
        )
        self.update(clat="India")
        self.assertEqual(
            self.plotter.ax.projection.proj4_params["lat_0"], 13.5
        )
    ...
    def test_grid_labelsize(self):
        ...
        try:
            texts = list(
                chain(
                    self.plotter.xgrid._gridliner.xlabel_artists,
            ...
            texts = list(
                chain(
                    self.plotter.xgrid._gridliner.label_artists,
...
def test_lonlatbox_projected():
    ...
    ax = sp.plotters[0].ax
    assert np.round(ax.get_extent(ccrs.PlateCarree()), 2).tolist() == [
        17.66,
        18.39,
        59.1,
        59.59,
    ...
    assert np.round(ax.get_extent(ccrs.PlateCarree()), 1).tolist() == [
        17.8,
        18.2,
        59.2,
        59.4,
...
def test_plot_curvilinear_datagrid(tmpdir):
    ...
    kws = dict(
        plot=None, xgrid=False, ygrid=False, map_extent="Czech Republic"
    )
